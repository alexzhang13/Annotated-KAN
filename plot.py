...
def plot(model: KAN, folder="./figures", beta=3, mask=False, mode="supervised", scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None):
        '''
        plot KAN
        ...
        for l in range(depth):
            ...
            for i in range(model.layer_widths[l]):
                for j in range(model.layer_widths[l + 1]):
                    ...
                    # numerical_mask = self.act_fun[l].mask.reshape(model.layer_widths[l + 1], model.layer_widths[l])[j][i]
                    ...
                    color = 'black'
                    plt.plot(model.layers[l].inp[:, i][rank].cpu().detach().numpy(), model.layers[l].activations[:, j, i][rank].cpu().detach().numpy(), color=color, lw=5)
                    # if sample == True:
                    ...
                    # lock_id = self.act_fun[l].lock_id[j * model.layer_widths[l] + i].long().item()
                    # if lock_id > 0:
        ...
        # if mode == "supervised":
        # alpha = [score2alpha(s.activations.cpu().detach().numpy()) for s in model.layers]
        # else:
        ...
        if in_vars != None:
            n = model.layer_widths[0]
            for i in range(n):
                plt.gcf().get_axes()[0].text(1 / (2 * (n)) + i / (n), -0.1, in_vars[i], fontsize=40 * scale, horizontalalignment='center', verticalalignment='center')

        if out_vars != None:
            n = model.layer_widths[-1]
            for i in range(n):
                plt.gcf().get_axes()[0].text(1 / (2 * (n)) + i / (n), y0 * (len(model.layer_widths) - 1) + 0.1, out_vars[i], fontsize=40 * scale, horizontalalignment='center', verticalalignment='center')

        if title != None:
            plt.gcf().get_axes()[0].text(0.5, y0 * (len(model.layer_widths) - 1) + 0.2, title, fontsize=40 * scale, horizontalalignment='center', verticalalignment='center')
